...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "📝 Отправь мне свою программу тренировок в формате:\n"
        "ПН: жим лёжа 3x10, присед 4x8; ВТ: подтягивания 3xмакс, тяга 4x10\n\n"
        "Или используй кнопки ниже для начала тренировки.",
...
async def start_training_button(message: Message):
    ...
    Проверяет наличие программы и перенаправляет в модуль тренировки.
    """
    user_id = message.from_user.id
    current_day = get_current_day()
    
    # Проверяем, есть ли программа на сегодня
    program = get_program(user_id, current_day)
    
    if not program or current_day not in program:
        await message.answer(
            f"❌ У тебя нет программы на сегодня ({current_day}).\n\n"
            "📝 Сначала отправь программу тренировок в формате:\n"
            "ПН: жим лёжа 3x10, присед 4x8; ВТ: подтягивания 3xмакс"
        )
        return
    
    # Импортируем и вызываем обработчик начала тренировки
...
async def handle_program_text(message: Message):
    ...
    try:
        ...
        # Сохраняем программу во временное хранилище (в реальности можно использовать Redis или FSM)
        # Для простоты будем использовать глобальный словарь
